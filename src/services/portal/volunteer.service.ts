...
export async function getMyProfile() {
    ...
    const { data, error } = await supabase
        .from('volunteers')
        .select('*')
        .eq('external_user_id', (ext as { id: string }).id)
        .maybeSingle();
...
// ─── updateBadge ───────────────────────────────────────────────────
// الإصلاح: تحويل من direct REST إلى RPC بـ SECURITY DEFINER
// السبب: جدول volunteers محمي بـ "Pure RPC Update Lockdown" (USING false)
// → أي update مباشر يفشل صامتاً دون رسالة خطأ واضحة
// → يجب استخدام دالة update_volunteer_badge (ghaith_patch_badge_rpc.sql)
export async function updateBadge(volunteerId: string) {
    const { data, error } = await supabase.rpc('update_volunteer_badge', {
        p_volunteer_id: volunteerId,
    });

    if (error) {
        console.error('updateBadge RPC Error:', error);
...
export async function generateCertificate(volunteerId: string): Promise<Blob> {
    const { data } = await supabase
        .from('volunteers')
        .select('volunteer_number,total_hours')
        .eq('id', volunteerId)
        .single();
...
<head>
  <meta charset="utf-8">
  <title>شهادة تطوع</title>
</head>
<body style="font-family:sans-serif;text-align:center;padding:2rem">
  <h1>شهادة تطوع</h1>
  <p>رقم المتطوع: ${d?.volunteer_number ?? volunteerId}</p>
  <p>إجمالي الساعات: ${d?.total_hours ?? 0}</p>
</body>
</html>`;
